...
4. **Push Better Auth env vars to Convex** — Convex functions don't read your `.env.local`; they have their own env:
   ...
   bunx convex env set GUEST_STAY_COOKIE_SECRET <value>
   ```

5. **Generate the guest Convex auth key**

   ```bash
   bun run generate:guest-auth-key
   ```

   Put the printed JSON into Convex env:

   ```bash
   bunx convex env set GUEST_CONVEX_AUTH_PRIVATE_JWK '<printed-json>'
   ```

   Guest JWT minting and JWKS generation both run in Convex. `auth.config.ts` inlines the public JWKS from `GUEST_CONVEX_AUTH_PRIVATE_JWK` (Convex cannot fetch JWKS from `localhost`). SvelteKit proxies `/api/guest-auth/token` and optionally `/api/guest-auth/jwks` for debugging.

   Guest auth env ownership:

   | Variable                        | Where     |
   | ------------------------------- | --------- |
   | `GUEST_STAY_COOKIE_SECRET`      | Convex    |
   | `GUEST_CONVEX_AUTH_PRIVATE_JWK` | Convex    |
   | `SITE_URL`                      | Convex    |
   | `PUBLIC_SITE_URL`               | SvelteKit |

   In production, `SITE_URL` and `PUBLIC_SITE_URL` must be the same public origin, including protocol and `www` choice.

6. **Configure Google OAuth**
   Authorized redirect URI: `${SITE_URL}/api/auth/callback/google` (e.g. `http://localhost:5173/api/auth/callback/google`).

7. **Run the dev server** (in a second terminal)
   ```bash
   bun run dev
   ```
...
src/
  convex/                    Convex functions + schema
    ...
    schema.ts                app tables (auditLogs, uploadedFilesR2)
