<script lang="ts">
	...
	const overviewItems = $derived([
		{
			id: 'active-venues',
			label: m['MyHospitalitiesPage.MyHospitalitiesHeader.activeVenues'](),
			value: summary?.activeCount ?? 0,
			icon: StoreIcon
		},
		{
			id: 'partnerships',
			label: m['MyHospitalitiesPage.MyHospitalitiesHeader.partnerships'](),
		...
		<div class="flex max-w-3xl min-w-0 flex-col gap-2">
			<div class="flex flex-wrap items-center gap-2">
				{#if !isLoading && !hasError}
					<Badge variant="outline">
						{m['MyHospitalitiesPage.MyHospitalitiesHeader.totalBadge']({ count: totalCount })}
					</Badge>
				{/if}
			</div>
			...
			<div>
				<h1 class="text-2xl font-semibold tracking-normal md:text-3xl">
					{m['MyHospitalitiesPage.MyHospitalitiesHeader.title']()}
				</h1>

				<p class="mt-2 text-sm leading-relaxed text-muted-foreground">
					{m['MyHospitalitiesPage.MyHospitalitiesHeader.description']()}
				</p>
			</div>
		</div>
