...
<div class="pl-0 lg:pl-4">
	<p
		class="mb-3 flex items-center gap-3 font-mono text-xs tracking-eyebrow text-primary uppercase sm:mb-(--gap-md)"
	>
		<span class="inline-block size-1.5 animate-pulse rounded-full bg-primary" aria-hidden="true"
		></span>
		<span>Step {demo.currentStep} of {DEMO_TOTAL_STEPS}</span>
	</p>

	<h2
		class="mb-4 font-display text-3xl leading-tight font-medium tracking-tight text-foreground sm:text-4xl"
	>
		{narr.title}
	</h2>

	<p
		class="min-h-lines-5 max-w-measure-narrow mb-6 text-base leading-relaxed text-muted-foreground"
	>
		{narr.body}
	</p>
	...
	<div class="mb-6 flex gap-1">
		{#each Array.from({ length: DEMO_TOTAL_STEPS }, (_, i) => i + 1) as n (n)}
			<button
				type="button"
				class="h-1 min-w-0 flex-1 cursor-pointer border-0 transition-colors not-first:ml-1 first:ml-0"
	...
	<div class="flex flex-wrap gap-3">
		...
		<Button variant="ghost" class="h-auto px-1.5 py-3 text-button" onclick={() => demo.goTo(1)}
			>Reset</Button
		>
	</div>
</div>
