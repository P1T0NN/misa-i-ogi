	...
	<div class="flex flex-1 flex-col justify-between gap-6 px-6 pt-14 pb-8">
		<div class="shadow-phone-float overflow-hidden rounded-2xl bg-(--surface-2) text-foreground">
			...
			<div class="px-5 pt-6 pb-5">
				<p class="mb-1 font-mono text-xs tracking-widest text-primary uppercase">
					{venue.vouchCat}
				</p>
				<h3 class="mb-1.5 font-display text-xl leading-snug font-medium">{venue.name}</h3>
				<p class="mb-4 text-xs text-muted-foreground">{venue.loc}</p>
				<div
					class="my-1 font-display text-voucher leading-none font-medium tracking-tight text-primary"
				>
				...
				<div class="flex items-center justify-between pt-1">
					<span class="font-mono text-xs tracking-caps text-muted-foreground uppercase"
						>Valid until</span
					>
					<span class="font-mono text-xs text-foreground tabular-nums">SUN 19 MAY · 14:00</span>
				</div>
				<div class="mt-2 flex items-center justify-between">
					<span class="font-mono text-xs tracking-caps text-muted-foreground uppercase">Code</span>
					<span class="font-mono text-xs text-foreground tabular-nums">{venue.code}</span>
				</div>
			</div>
		</div>
