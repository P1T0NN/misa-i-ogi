<script lang="ts">
	...
	async function requestAccess() {
		...
		try {
			...
			errorMessage = m['ScanPage.ScanShareAccess.unavailable']();
		} finally {
			isPending = false;
		}
	}
...
<Card.Root class="gap-5 rounded-xl border border-border bg-card p-6 shadow-none">
	<Card.Header class="gap-2 p-0">
		<Card.Title class="font-serif text-xl font-medium text-foreground">
			{#if activeScan}
				{m['ScanPage.ScanShareAccess.activeTitle']()}
			{:else}
				{m['ScanPage.ScanShareAccess.title']()}
			{/if}
		</Card.Title>
		<Card.Description class="text-sm leading-relaxed text-muted-foreground">
			{#if activeScan}
				{m['ScanPage.ScanShareAccess.body']()}
			{:else}
				{m['ScanPage.ScanShareAccess.directBody']()}
			{/if}
		</Card.Description>
	</Card.Header>
		...
		<div class="flex flex-col gap-4" aria-busy={isPending}>
			<Field data-invalid={isInvalid}>
				<FieldLabel for="stay-sharing-code-{id}">
					{m['ScanPage.ScanShareAccess.label']()}
				</FieldLabel>
				<Input
					...
					disabled={isPending}
					placeholder={m['ScanPage.ScanShareAccess.placeholder']()}
					aria-invalid={isInvalid ? 'true' : undefined}
					oninput={handleCodeInput}
					onkeydown={handleCodeKeydown}
				/>
				<FieldDescription>{m['ScanPage.ScanShareAccess.hint']()}</FieldDescription>
				{#if errorMessage}
			...
			<Button type="button" class="w-full" onclick={requestAccess} disabled={isPending}>
				{#if isPending}
					<Spinner />
				{/if}
				{m['ScanPage.ScanShareAccess.submit']()}
			</Button>
		</div>
	</Card.Content>

	<Card.Description class="text-xs leading-relaxed text-muted-foreground">
		{m['ScanPage.ScanShareAccess.scanInstead']()}
	</Card.Description>
</Card.Root>
