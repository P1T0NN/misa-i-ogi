 ...
 */
export const createUser = adminMutation('createUser')({
	args: {
		name: v.string(),
		email: v.string(),
		password: v.string(),
	...
	handler: async (ctx, args): Promise<MutationResult> => {
		...
		if (!name) {
			return { success: false, message: { key: 'ValidationMessages.SignUpForm.nameRequired' } };
		}
		...
		if (args.password.length < MIN_PASSWORD_LENGTH) {
			return {
				success: false,
				message: {
					key: 'GenericMessages.PASSWORD_TOO_SHORT',
		...
		const created = await auth.api.createUser({
			body: {
				...
				name,
				role: args.role,
				data: { emailVerified: args.emailVerified }
			},
			headers
		});
		...
		ctx.audit(AUDIT_ACTIONS.USER_CREATE, {
			resource: { table: 'user', id: user.id },
			after: {
				email: user.email ?? email,
				role: user.role ?? args.role,
				emailVerified: user.emailVerified ?? args.emailVerified
			}
		});

		return { success: true, message: { key: 'GenericMessages.USER_CREATED' } };
	}
});
