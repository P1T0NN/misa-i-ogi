 ...
 * a `TranslatableMessage` ΓÇö the client picks it up via `error.data.message` and feeds it
 * straight into `translateFromBackend` for locale-aware toast text.
 *
 * - `NOT_AUTHENTICATED` ΓÇö no session at all
 * - `ADMIN_ACCESS_REQUIRED` ΓÇö signed in but `role !== 'admin'`
 *
 * Returns the caller's better-auth user id ΓÇö the Convex `_id` of the user row.
 ...
 * pre-filled from the caller. IP / UA are intentionally NOT plumbed through ΓÇö
 ...
 * - Injects `ctx.userId: Id<'users'>` into the handler ΓÇö no need to re-auth inside.
 *
 * @param name  Function name ΓÇö must match the export name and a key in {@link convexRateLimitRegistry}.
 *
 * @example
 * ```ts
