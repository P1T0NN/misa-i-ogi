 ...
 * Pick `'offset'` only when you need page-number jumps or an accurate total ΓÇö and only on
 ...
 * search modes ΓÇö computing it would defeat the optimization.
 */
...
// ΓöÇΓöÇΓöÇ Access patterns ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
 ...
 * (JS preserves object insertion order ΓÇö caller controls it), and only a strict prefix
 ...
 * supported ΓÇö `.filter()` is post-scan and silently degrades to O(table) + thin pages.
 */
 ...
 * Search results are relevance-ordered ΓÇö `order` is ignored when `search` is supplied.
...
// ΓöÇΓöÇΓöÇ Builder option types ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
/** Builder fn ΓÇö receives ctx + the merged (builtin + caller) args, returns access spec. */
 ...
 *
 * - `'user'` ΓÇö caller must have an authenticated session; throws `NOT_AUTHENTICATED`.
 * - `'admin'` ΓÇö caller must be authenticated AND have `role === 'admin'`; throws
 ...
 * {@link getAuthUserId} + ctx reads ΓÇö `auth` is the cheap whole-endpoint guard.
 */
...
export type FetchOptimizedOptions<T extends TableNames, Extra extends PropertyValidators> = {
	 ...
	 *
	 * Pass a function `(args) => 'asc' | 'desc'` to derive the direction at request time ΓÇö
	 ...
	 * (full table walk along `_creationTime` ΓÇö only OK on small tables). Mutually
...
// ΓöÇΓöÇΓöÇ Internal helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
 ...
 * declaration order ΓÇö the helper does not (cannot) reorder for you.
 */
 ...
 * });
 *
 * // 4) Full-text search ΓÇö pass the function name for advisory rate limiting and pair
 ...
 *
 * 2. **Index-only filtering.** `where` compiles to `withIndex` ΓÇö O(matching rows). The
 ...
 *    subscription ΓÇö paged history hits the query cache; only the live page re-evaluates on
 *    writes.
 *
 * 5. **Offset mode kept honest.** Still O(rows) ΓÇö that's unavoidable without an aggregate.
 ...
 *
 * Cursors are opaque tokens computed against a specific access spec ΓÇö `where` eq values,
 ...
 * meaningless against the new one. Convex won't crash ΓÇö it returns nothing useful.
 *
 ...
 * itself can't know ΓÇö it sees each request in isolation.
 *
 * ## Auth + rate-limit
 *
 * Three layers, composable:
 *
 *   - `auth` ΓÇö endpoint-level gate. `'user'` requires any session, `'admin'`
 *     requires `role === 'admin'`. Cheapest and earliest check.
 *   - Function name (rate-limited overload) ΓÇö advisory rate-limit via `convexRateLimiter.check`.
 *     Always pair with `auth: 'user'` (or `'admin'`) so the bucket key is per-user.
 *     **Strongly recommended for `search` endpoints.**
 *   - `where` builder ΓÇö row-level rules. Read auth/userId/roles inside the builder and use
...
>(options: FetchOptimizedOptions<T, Extra>, rateLimitName: ConvexRateLimitName | null) {
	...
	if (search && strategy === 'offset') {
		// Convex search indexes don't support `.collect()` ΓÇö only `.paginate()`. Fail fast at
		// factory build time so this surfaces during dev, not on the first prod request.
		throw new Error(
			`[fetchOptimized:${table}] 'search' requires 'strategy: cursor' ΓÇö Convex search indexes are paginate-only.`
		);
	}
	...
	return query({
		...
		handler: async (ctx: QueryCtx, rawArgsRaw): Promise<FetchOptimizedResult<T>> => {
			...
			if (auth === 'admin') {
				...
				// rate-limit bucket when possible ΓÇö fall back to the auth subject if available.
				authedUserId = await getAuthUserId(ctx);
			}
			...
			//    access by inspecting `args` ΓÇö but only one may be active per request, since
			...
			if (whereSpec && searchSpec) {
				throw new Error(
					`[fetchOptimized:${table}] both 'where' and 'search' resolved to a spec ΓÇö return null from one of them based on args (typically: search active when args.search non-empty, where otherwise).`
				);
			}
